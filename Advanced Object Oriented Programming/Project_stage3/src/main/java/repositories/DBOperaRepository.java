...
public class DBOperaRepository implements SpectacolRepository<Opera> {
    ...
    public void deleteSpectacolByNume(String nume) {
        String sql = "DELETE FROM opere WHERE nume = '"+nume+"' ";
        try (
                Connection con = DBConnectionManager.getInstance().createConnection();
    ...
    public void updateOraSpectacol(int ora_veche, int ora) {
        String sql = "UPDATE opere SET ora = '"+ora+"' WHERE ora = '"+ora_veche+"' ";
        try (
                Connection con = DBConnectionManager.getInstance().createConnection();
                Statement statement = con.createStatement();
        ) {
